...
class CurrentUser with ChangeNotifier {
  ...
  static Future initialize() async {
    ...
    if (dataList.length >= 1) {
      ...
      location.name = lastUser['locationName'] != null ? lastUser['locationName'] : '';
      location.cityId = lastUser['locationCityId'] != null ? lastUser['locationCityId'] : '';
      location.cityName = lastUser['locationCityName'] != null ? lastUser['locationCityName'] : '';
      location.stateId = lastUser['locationStateId'] != null ? lastUser['locationStateId'] : '';
      location.stateName = lastUser['locationStateName'] != null ? lastUser['locationStateName'] : '';
      location.cityState = lastUser['locationCityState'] != null ? lastUser['locationCityState'] : '';
      location.countryCode = lastUser['countryCode'] != null ? lastUser['countryCode'] : '';
      print('$id $name $username $email $picture $isLoggedIn');
      if (location.name == '' && location.cityId == '' && location.cityName == '' && location.stateId == '' && location.stateName == '' && location.cityState == '') {
