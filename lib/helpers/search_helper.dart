...
class LocationSearch extends SearchDelegate<String> {
  ...
  Widget buildSuggestions(BuildContext context) {
    ...
    return StatefulBuilder(builder: (BuildContext statCtx, StateSetter setState) {
      ...
      if (_isState) {
        ...
        LocationProvider.cities.forEach((element) {
          if (element.name.contains(query) && element.stateId == _chosenStateCode) {
            suggestions.add(element);
          }
        });
      }
          ...
          ListTile(
            ...
            onTap: () async {
              if (!CurrentUser.uploadingAd) {
                ...
                Navigator.pushNamedAndRemoveUntil(context, TabsScreen.routeName, (Route<dynamic> route) => false);
              } else {
                CurrentUser.uploadingAd = false;
                CurrentUser.prodLocation.name = LocationProvider.countries[0].name;
              ...
              child: ListView.builder(
                ...
                itemBuilder: (content, i) => ListTile(
                  ...
                  onTap: () async {
                    ...
                    if (LocationProvider.cities.where((element) => _chosenStateCode == element.stateId).toList().length == 0) {
                      if (!CurrentUser.uploadingAd) {
                        ...
                        CurrentUser.prodLocation.stateName = suggestions[i].name;
                        CurrentUser.prodLocation.cityState = '';
                        CurrentUser.uploadingAd = false;
                ...
                itemBuilder: (content, i) => ListTile(
                  ...
                  onTap: () async {
                    if (!CurrentUser.uploadingAd) {
                      ...
                      CurrentUser.prodLocation.stateName = suggestions[i].stateName;
                      CurrentUser.prodLocation.cityState = suggestions[i].cityState;
                      CurrentUser.uploadingAd = false;
                      close(context, suggestions[i].name);
                    }
                  },
