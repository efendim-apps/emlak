...
import 'package:eRealState_App/models/product_old.dart';
...
class APIHelper extends ChangeNotifier {
  ...
  static const BASE_URL = "emlak.efendim.biz";
  ...
  static const SUPPORT_EMAIL = "support@canders.in";
  ...
  Future loginUserUsingEmail(
    ...
    if (response.statusCode == 200) {
      ...
      if (decodedResponse['status'] == 'success') {
        ...
        throw Exception(decodedResponse["message"] ?? "Error");
      }
    } else {
  ...
  Future fetchProducts({status, countryCode, state, city, page, limit}) async {
    ...
    final String url = 'https://$BASE_URL/$UNENCODED_PATH?action=$PRODUCT_LIST_URL&status=active&country_code=$countryCode&state=$state&city=$city&page=$page&limit=$limit';
    print(url);
    final client = new http.Client();
    final response = await client.get(Uri.parse(url), headers: {HttpHeaders.contentTypeHeader: 'application/json'},);
  ...
  Future fetchFeaturedAndUrgentProducts({Location userLocation}) async {
    ...
    final String url = 'https://$BASE_URL/$UNENCODED_PATH?action=$FEATURED_URGENT_LIST_URL&status=active&city=${userLocation.cityId}&country_code=${userLocation.countryCode}&state=${userLocation.stateId}';
    print(url);
    final client = new http.Client();
    final response = await client.get(Uri.parse(url), headers: {HttpHeaders.contentTypeHeader: 'application/json'},);
  ...
  Future getCustomDataByCategory({
    ...
    final url = await Uri.https(BASE_URL, UNENCODED_PATH, {
  ...
  Future updateProfilePic({
    ...
    if (imageFile != null) {
      ...
      await request.files.add(await http.MultipartFile.fromPath(
        'fileToUpload',
        imageFile.path,
      ));
  ...
  Future fetchPaymentVendorCredentials() async {
    ...
    https://emlak.efendim.biz/api/v1/index.php?action=payment_api_detail_config
    https://emlak.efendim.biz/api/v1/index.php?action=payment_api_detail_config
    final response = await http.get(url);
    if (response.statusCode == 200) {
      final decodedData = json.decode(response.body);
